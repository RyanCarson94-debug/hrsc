...
export default function GenerateTab({ state }) {
  ...
  function onEmpNext() {
    ...
    const av = { employee_name:emp.employee_name, company_name:emp.company_name, job_title:emp.job_title, business_unit:emp.businessUnit, country:emp.country };
    res.forEach(s => {
  ...
  const neededVars = useMemo(() => {
    ...
    resolved.forEach(s => {
      ...
      (t.match(/\{\{(\w+)\}\}/g)||[]).forEach(m => {
        const k = m.slice(2,-2);
        if (!seen.has(k)) { seen.add(k); const v=cl?.variables.find(v=>v.key===k); r.push({key:k,label:v?.label||k.replace(/_/g," "),type:v?.type||"text"}); }
      });
    });
    return r;
  }, [resolved, clauses]);
...
${resolved.map((s,i) => {
  ...
  return `<h2>${prefix}${s.name}</h2><p>${rendered.replace(/\n\n/g,"</p><p>").replace(/\n/g,"<br>")}</p>`;
    ...
    a.download = `${(tmpl.name+" "+(emp.employee_name||"draft")).replace(/\s+/g,"_")}.doc`;
    a.click();
  }
            ...
            {fired.length===0
              ...
              : fired.map(r => (
                  <div key={r.id} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"7px 0",borderBottom:`1px solid ${B.g1}`}}>
                    <span style={{width:6,height:6,borderRadius:"50%",background:B.teal,flexShrink:0,marginTop:4}}/>
                    <div><div style={{fontSize:12,fontWeight:600}}>{r.name}</div><div style={{fontSize:11,color:B.g3}}>{r.description}</div></div>
                  </div>
                ))
            }
          </div>
