...
function AuditLog() {
  ...
  }, [filterAction, filterType, filterUser, page]);
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", gap:10, marginBottom:16, flexWrap:"wrap" }}>
        ...
        <input style={{...inp,width:180}} value={filterUser} onChange={e=>{setFilterUser(e.target.value);setPage(0);}} placeholder="Filter by user…"/>
        <button style={{...BS,padding:"7px 14px",fontSize:12}} onClick={()=>{setFilterAction("");setFilterType("");setFilterUser("");setPage(0);}}>Clear</button>
      </div>
      ...
      <div style={{ display:"flex", gap:8, justifyContent:"center", marginTop:16 }}>
        ...
        <span style={{ fontSize:12, color:B.g3, alignSelf:"center" }}>Page {page+1}</span>
        ...
        <div key={g.id} style={CARD({marginBottom:8})}>
          ...
          {expanded===g.id && (
            <div style={{ marginTop:14, paddingTop:14, borderTop:`1px solid ${B.g2}` }}>
              <div style={{ fontSize:10, fontWeight:700, letterSpacing:"0.07em", textTransform:"uppercase", color:B.g3, marginBottom:8 }}>Details</div>
              <div style={{ fontSize:12, color:B.g3 }}>
                <div style={{ marginBottom:4 }}><strong>Template:</strong> {g.template_name}</div>
                <div style={{ marginBottom:4 }}><strong>Employee:</strong> {g.employee_name}</div>
                <div style={{ marginBottom:4 }}><strong>Country:</strong> {g.country || "—"}</div>
                <div style={{ marginBottom:4 }}><strong>Generated by:</strong> {g.user_name || "—"}</div>
                <div><strong>Generated at:</strong> {formatDate(g.generated_at)}</div>
              </div>
            </div>
          )}
        </div>
